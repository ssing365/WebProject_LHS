...
public class DBConnPool {
	...
	public DBConnPool() {
		try {
			 ...
			 * */
			Context ctx = (Context)initCtx.lookup("java:comp/env"); //C:\Developkits\apache-tomcat-10.1.31\conf\context.xml
			/*
			 3. server.xml에 등록한 네이밍을 Lookup하여 DataSource를 얻어온다.
			 즉 DB연결을 위한 정보를 가지고 있다.*/
			DataSource source = (DataSource)ctx.lookup("dbcp_myoracle"); //server.xml
